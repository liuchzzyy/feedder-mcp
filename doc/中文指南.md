...
CLI (fetch --source gmail) → GmailSource
    ├─ _write_json_configs()  →  token.json + credentials.json
...
class PaperItem(BaseModel):
    ...
    doi: Optional[str] = None                           # DOI（可选，无前缀）
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict)  # 可扩展元数据
```

**约束：**
- `title`、`source` 和 `source_type` 始终必填
- `doi` 可选（`None`），不是空字符串 — enrichment 阶段可能补充
- `metadata` 是来源特定数据的扩展字段 — 应优先使用它而非修改模型 schema（如 `metadata["crossref"]`、`metadata["feed"]`）
...
- **构造函数**: `GmailSource(query?, source_name?, max_results?, mark_as_read?, auto_detect_source?, processed_label?)`
  - 验证 `GMAIL_TOKEN_JSON` 和 `GMAIL_CREDENTIALS_JSON` 必填（`ValueError` if missing）
  - 使用固定文件路径 `token.json` / `credentials.json`（内部实现细节）
- **依赖**: `ezgmail`, `GmailParser`
- **延迟初始化**: `_ensure_init()` → `_write_json_configs()`（将内联 JSON 写入磁盘文件）→ `ezgmail.init(tokenFile, credentialsFile)`
...
- **附件发现**: `_extract_attachment_info()` — 从 `message._attachmentsInfo` / `message.attachments` 中提取 PDF 文件名
- **去重**: `_deduplicate()` — 按标题（不区分大小写）跨所有邮件去重
- **功能**: 日期过滤、标记已读（`thread.markAsRead()`）、处理标签（`message.addLabel()`）、失败标记未读（`thread.markAsUnread()`）
...
3. 运行 ezgmail 初始化获取 token.json
4. 将两个 JSON 文件内容作为字符串设置到 `.env`

**注意**: JSON 值在启动时自动写入固定文件路径（token.json / credentials.json）以兼容 EZGmail。这些文件已在 `.gitignore` 中，不会被提交。
...
```bash
# 检查清单：
# 1. GMAIL_TOKEN_JSON 和 GMAIL_CREDENTIALS_JSON 都已设置
