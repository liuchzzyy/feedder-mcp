...
class TestBuildParser:
    ...
    def test_fetch_parser_rejects_non_positive_limit(self):
        parser = _build_parser()
        with pytest.raises(SystemExit):
            parser.parse_args(["fetch", "--limit", "0"])
        with pytest.raises(SystemExit):
            parser.parse_args(["fetch", "--limit", "-1"])
    ...
    def test_enrich_parser_rejects_non_positive_concurrency(self):
        parser = _build_parser()
        with pytest.raises(SystemExit):
            parser.parse_args(["enrich", "--input", "in.json", "--concurrency", "0"])
        with pytest.raises(SystemExit):
            parser.parse_args(["enrich", "--input", "in.json", "--concurrency", "-3"])
...
class TestHandleFilter:
    ...
    @pytest.mark.asyncio
    async def test_handle_filter_without_keywords_does_not_autogenerate_when_other_filters_present(
        self, sample_papers_json, tmp_path
    ):
        output_file = tmp_path / "filtered.json"
        ...
        with patch(
            ...
            with patch("src.filters.pipeline.FilterPipeline") as mock_pipeline_class:
                ...
                mock_keyword_gen.assert_not_called()
                call_args = mock_pipeline.filter.call_args
                criteria = call_args[0][1]
...
class TestHandleEnrich:
    ...
    @pytest.mark.asyncio
    async def test_handle_enrich_invalid_concurrency_exits(self, sample_papers_json, tmp_path):
        args = argparse.Namespace(
            input=str(sample_papers_json),
